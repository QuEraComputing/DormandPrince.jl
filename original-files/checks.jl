...
function check_uround(options::DP5Options{T}) where T
...
function check_beta(options::DP5Options{T}) where T
    if options.beta > 0.2
        return false
    else
        return true
...
function check_safety_factor(options::DP5Options{T}) where T
